...
void MiniGL::drawMesh(const TriangleMesh &mesh, const float * const color)
{
	...
	const Vector3r *vertices = mesh.getVertices().data();
	const Vector3r *vertexNormals = mesh.getVertexNormals().data();

	if (MiniGL::checkOpenGLVersion(3, 3))
	{
		glEnableVertexAttribArray(0);
		glVertexAttribPointer(0, 3, GL_REAL, GL_FALSE, 0, &mesh.getVertices()[0]);
...
void MiniGL::setPosition(Real x, Real y, Real z)
{
	m_translation[0] = x;
	m_translation[1] = y;
	m_translation[2] = z;
