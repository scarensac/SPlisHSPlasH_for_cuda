...
void DFSPHCUDA::step()
{
	...
	//load the result to the normal program (not done int he version with both of the codes running
	//m_data.readDynamicData(m_model, m_simulationData);



	for (int i = 0; i < m_data.numFluidParticles; ++i) {
		checkVector3("end check x:", m_model->getPosition(0, i), m_data.posFluid[i]);
		checkVector3("end check v:", m_model->getVelocity(0, i), m_data.velFluid[i]);
	
		Real &densityAdv = m_data.densityAdv[i];
		const Vector3d &xi = m_data.posFluid[i];
		const Vector3d &vi = m_data.velFluid[i];
		densityAdv = 0.0;
		unsigned int numNeighbors = m_data.getNumberOfNeighbourgs(i);
		...
		for (unsigned int j = 0; j < numNeighbors; j++)
		{
			...
			const Vector3d &vj2 = m_data.velFluid[neighborIndex];
			checkVector3("end check vj:", vj, vj2);
			checkVector3("end check vi:", vector3dTo3r(vi), vi);
			checkVector3("end check vi-vj:", vi - vector3rTo3d(vj), vi - vj2);
		}
...
void DFSPHCUDA::computeDFSPHFactor()
{
	...
	cuda_computeDFSPHFactor(m_data);
...
void DFSPHCUDA::divergenceSolve()
{
	...
	//std::cout << "h old: " << m_data.h_past << std::endl;
	//std::cout << "h start: " << h << std::endl;

	for (int i = 0; i < m_data.numFluidParticles; ++i) {
		checkVector3("divergence start check x:", m_model->getPosition(0, i), m_data.posFluid[i]);
		checkVector3("divergence start chack v:", m_model->getVelocity(0, i), m_data.velFluid[i]);
	}
	...
	#pragma omp parallel default(shared)
	{
		...
		for (int i = 0; i < numParticles; i++)
		{
			...
			checkReal("divergence: warm start: kappaV start loop:", m_simulationData.getKappaV(i), m_data.kappaV[i]);
		}
		...
		for (int i = 0; i < (int)numParticles; i++)
		{
			...
			checkVector3("divergence: warm start: vel second loop:",m_model->getVelocity(0, i), m_data.velFluid[i]);
		}
	...
	#pragma omp parallel default(shared)
	{
		...
		for (int i = 0; i < (int)numParticles; i++)
		{
			...
			checkReal("divergence: loop innitialization: computeDensityChange:", m_simulationData.getDensityAdv(i), m_data.densityAdv[i]);
		}
	}
...
{
	{
		...
		for (unsigned int j = 0; j < numNeighbors; j++)
		{
			...
			const Vector3r &vj = m_model->getVelocity(0, neighborIndex);
			const Vector3d &vj2 = m_data.velFluid[neighborIndex];
			checkVector3("computeDensityChange vj:", vj, vj2);
			checkVector3("computeDensityChange vi:", vector3dTo3r(vi), vi );
			checkVector3("computeDensityChange vi-vj:", vi - vector3rTo3d(vj), vi - vj2);
			densityAdv += m_model->getMass(neighborIndex) * (vector3dTo3r(vi) - vj).dot(m_model->gradW(vector3dTo3r(xi - xj)));
		}
...
void DFSPHCUDA::updateVelocities(double h) 
{
	cuda_updateVelocities(m_data);
...
void DFSPHCUDA::updatePositions(double h)
{
	cuda_updatePositions(m_data);
...
void DFSPHCUDA::updateTimeStepSizeCFL(const Real minTimeStepSize)
{
