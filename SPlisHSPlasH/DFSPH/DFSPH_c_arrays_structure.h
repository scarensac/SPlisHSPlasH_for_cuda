	...
	*/
	...
	public:
		 Real W(const Real r) const
		{
		 ...
		 Real W(const Vector3d &r) const
		{
			return W(r.norm());
		}

		 Vector3d gradW(const Vector3d &r) const
		{
			Vector3d res;
...
#define MAX_NEIGHBOURS 60
		...
		Real W(const Vector3d &r) const { return m_kernel.W(r); }
		Vector3d gradW(const Vector3d &r) { return m_kernel.gradW(r); }
		Real W_zero;
		double density0;
		...
		inline unsigned int getNeighbour(int particle_id, int neighbour_id, int body_id = 0) {
			return neighbourgs[body_id*numFluidParticles*MAX_NEIGHBOURS + particle_id * MAX_NEIGHBOURS + neighbour_id];
		}

		inline unsigned int getNumberOfNeighbourgs(int particle_id, int body_id = 0) {
